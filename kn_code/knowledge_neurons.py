...
class KnowledgeNeuronsBase:
    def __init__(
        ...
        if 'llama' == model_type or 'Qwen' == model_type or 'gemma' == model_type:
            print("full device map (multi-gpus):", model.hf_device_map, model_type)
        ...
        # elif 'llama' == model_type or 'Qwen' == model_type:
        elif 'llama' == model_type or 'Qwen' == model_type or 'gemma' == model_type: # gemma 2 as well
    ...
    def _prepare_inputs(self, prompt, target=None, encoded_input=None):
        if encoded_input is None:
            ####### prompt modify for llama
            if 'llama' in self.model_type or 'Qwen' in self.model_type or 'gemma' in self.model_type:
                print('kn chat template loaded...')
                if 'gemma' in self.model_type:
                    print('gemma 2 chat template loaded...')
                    m = [
                    {
                        "role": "user",
                        "content": prompt
                    ...
                    print('normal chat template loaded...')
                    m = [
                    {
                        "role": "system",
                ...
                encoded_input = self.tokenizer.apply_chat_template(m, add_generation_prompt=True, return_tensors="pt", return_dict=True).to(self.device)
                print('chat template loading...')
            else:
        ...
        if target is not None:
            ...
            if "gpt" in self.model_type or 't5' in self.model_type:
                target = self.tokenizer.encode(target)
            elif 'llama' in self.model_type or 'Qwen' in self.model_type or 'gemma' in self.model_type: ## add up here, need to test other llms then
    ...
    def _generate(self, prompt, ground_truth):
        ...
        n_sampling_steps = len(target_label) if ("gpt" in self.model_type or 'llama' in self.model_type or 'Qwen' in self.model_type or 'gemma' in self.model_type) else 1
        all_gt_probs = []
        all_argmax_probs = []
        argmax_tokens = []
    ...
    def get_scores(
        ...
        scores = []
        # encoded_input = self.tokenizer(prompt, return_tensors="pt").to(self.device) # original
        encoded_input = None # chat template in neuron seeking
    ...
    def get_scores_for_layer(
        ...
        if attribution_method == "integrated_grads":
            ...
            for i in range(n_sampling_steps):
                if i > 0 and (self.model_type == "gpt" or self.model_type == 'llama' or self.model_type == 'Qwen' or 'gemma' in self.model_type):
                    # retokenize new inputs
                    encoded_input, mask_idx, target_label = self._prepare_inputs(
                        prompt, ground_truth
                    )
                ...
                # scaled_weights = scaled_weights.to(model_dtype)
                # ###


                integrated_grads_this_step = []  # to store the integrated gradients

                # print('L543 scaled weights: ', scaled_weights, scaled_weights.shape)

                for batch_weights in scaled_weights.chunk(n_batches):
                    batch_weights = batch_weights.detach().clone().to(torch.float32).requires_grad_(True) ###
                    # print('L546 batch weights: ', batch_weights, batch_weights.shape)
                    ...
                    # ###
                    # probs = F.softmax(outputs.logits[:, mask_idx, :], dim=-1) # ori
                    probs = F.softmax(outputs.logits[:, mask_idx, :], dim=-1, dtype=torch.float32)


                    # ######
                    # # 1) cast logits involved in softmax to fp32 for numerical stability
                    # logits = outputs.logits[:, mask_idx, :].to(torch.float32)

                    # # 2) numerically stable softmax: subtract per-row max BEFORE softmax
                    # logits = logits - logits.max(dim=-1, keepdim=True).values

                    # probs = F.softmax(logits, dim=-1)
                    # ######

                    assert torch.isfinite(baseline_activations).all(), "baseline_activations has NaN/Inf"
                    # after forward:
                    assert torch.isfinite(outputs.logits).all(), "logits has NaN/Inf"

                    assert torch.isfinite(probs).all(), "probs has NaN/Inf"

                    # print('L593 probs: ', probs, probs.shape)
                    target_idx = (
                    ...
                    # print('L613 integrated grads this step: ', integrated_grads_this_step)
                    ...
                    # del logits, probs, grad, batch_weights
                    del probs, grad, batch_weights
                    torch.cuda.empty_cache()
                ...
                del integrated_grads_this_step, scaled_weights, baseline_activations
                torch.cuda.empty_cache()
    ...
    def enhance(self, neurons, val):
        ...
        patch_ff_layer(
            ...
            ff_attrs=self.input_ff_attr,
            _value=val,
            # output_ff_attr=self.output_ff_attr,
        )
        modified_layers = list(set([x[0] for x in neurons]))
        return modified_layers

class KnowledgeNeurons(KnowledgeNeuronsBase):
    ...
    def search_kns_modi(self, source, target, n_prompts=100, batch_size=20, verbose=False):
        att_scores = []
        att_score = self.get_scores(source, target, batch_size=batch_size, pbar=False, steps=20)
        if verbose:
            print(f"Source: {source}\nTarget: {target}\nScores: {att_score}")
        return att_score.detach()

    def get_raw_n(self, att_scores, threshold=0.2):
        att_scores = torch.stack(att_scores)
        raw_neurons = fix_p_neuron(att_scores, threshold=threshold)
        print('fix p neurons raw: ', raw_neurons)
        return raw_neurons
    
    def get_raw_n_non_fix(self, att_scores, threshold=0.2):
        att_scores = torch.stack(att_scores)
        raw_neurons = run_kn_alg(att_scores)
        return raw_neurons

    def get_kns_func(self, att_scores, threshold=0.2, p=0.7):
        # check above
        att_scores = torch.stack(att_scores)
        print('=== START ===')
        # print('TARGET', target)
        group_neurons = fix_p_neuron(att_scores, threshold=threshold, p=p)
        print('fix p neurons: ', group_neurons)
        print(run_kn_alg(att_scores))
        print(cosine_sim_n(att_scores.view(att_scores.shape[0], -1)))
        print('=== END ===')
        return group_neurons
...
def fix_p_neuron(scores, threshold=0.2, p=0.7):
    coarse_neurons = []
    for score in scores:
        _threshold = score.max().item() * threshold
        coarse_neurons.append(torch.nonzero(score > _threshold).tolist())
